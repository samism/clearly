...
**The hidden titlebar still intercepts clicks.** With `titleVisibility = .hidden` + `titlebarAppearsTransparent`, `NSTitlebarView.hitTest` returns itself for the empty strip, so SwiftUI chrome drawn in the top band (tab capsules, sidebar toggle, new-tab button) never receives mouse events — while hover, tooltips, and accessibility AXPress still work, masking the breakage. `TitlebarClickThroughInstaller` (FolderSidebarView.swift) wraps `hitTest` to pass empty-strip clicks through on windows flagged with `hypergraphiaTitlebarClicksPassThrough`; `configureDocumentWindowChrome` sets the flag on the macOS 26 glass-chrome path only. Any new interactive chrome in the titlebar band depends on this staying installed.
